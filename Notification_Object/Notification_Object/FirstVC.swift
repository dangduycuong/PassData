...
class FirstVC: UIViewController {
    ...
    @objc func receivedData(_ notification: Notification) {
        firstLabel.text = notification.object as? String
    }
    
    @IBAction func registerButton(_ sender: Any) {
        let secondVC = storyboard?.instantiateViewController(withIdentifier: "secondVCID")
        NotificationCenter.default.addObserver(self, selector: #selector(receivedData(_:)), name: NSNotification.Name(rawValue: "key"), object: nil)
